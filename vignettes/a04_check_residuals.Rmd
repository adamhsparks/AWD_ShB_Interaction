...
output:
  html_document:
    ...
    theme: journal
vignette: >
  %\VignetteIndexEntry{Check Data Residuals}
...
AUDPS_15 <- subset(AUDPC_S, YEAR == 2015)
AUDPS_16 <- subset(AUDPC_S, YEAR == 2016)
```
...
Based on these graphs, few of the data are a good fit for a normal distribution.
Ideally, we want to see the residuals close to the solid line and between the dotted lines.
Several of the data do not exhibit this behaviour, which suggests that using an analysis that does need these assumptions to be met, _e.g._ a Bayesian analysis method could be used.
The _MCMCglmm_ package provides an easy way to implement this type of analysis.
