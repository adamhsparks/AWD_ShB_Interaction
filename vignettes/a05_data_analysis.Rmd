...
Based on the residual plots where several do not meet assumptions for a normal distribution, I've elected to use `MCMCglmm()` for a split plot analysis rather than a `lmer()` model.
The `MCMCglmm()` model will work for all the data regardless of the residuals.
...
Fitting the models with the default iterations resulted in some less than acceptable models.
Because of this, the number of iterations has been increased, `nitt = 55000,` and `burnin = 5000`.
This results in a sample size of 5000 and much smaller errors for random effects and better models.
...
AUDPS_2015 <- as.data.frame(AUDPC_S[AUDPC_S$YEAR == 2015, ])
...
AUDPS_2016 <- as.data.frame(AUDPC_S[AUDPC_S$YEAR == 2016, ])
...
As `MCMCglmm` is a Bayesian analysis, it requires priors to be set.
Here we set flat weakly-informative priors that have a uniform low degree of belief in all parameters for all six models we will create since we don't have good prior knowledge of what the curves should look like.
...
Tiller sheath blight incidence AUDPS, `TShB_inc_AUDPS` is the response variable.
...
Tiller sheath blight severity AUDPS, `TShB_sev_AUDPS` is the response variable.
The interaction of water management, `WMGT`, and nitrogen rate, `NRTE`, are fixed effects.
Replicate, `REP` is treated as a rando
...
Leaf sheath blight severity AUDPS, `LShB_sev_AUDPS` is the response variable.
The interaction of water management, `WMGT`, and nitrogen rate, `NRTE`, are fixed effects.
Replicate, `REP` is treated as a rando
...
In 2015 both the N100 and N120 rates were significantly different than the N0 treatment
. The AWD water management was not significantly different from the base PDL treatment.

In 2016 the N180 was significantly different from the N60 rate.
The AWD water management was also significantly different from the base PDL treatment.

## Leaf Sheath Blight Severity

In 2015 both the N100 and N120 rates were significantly different than the N0 treatment.
The AWD water management was not significantly different from the control PDL treatment.
In the leaf phase of the disease neither N by WMGT interaction was significant.
...
The N180 by AWD interaction was significantly different from the N60 by PDL water management.
