...
The `AUDPS` object is loaded with the `rice.awd.shb` R package. To see how the AUDPC data were generated from the original raw data, see the [https://github.com/openplantpathology/rice_awd_pests/blob/master/data-raw](https://github.com/openplantpathology/rice_awd_pests/blob/master/data-raw) file.
...
Here we set flat non-informative priors that have a uniform low degree of belief in all parameters for all six models we will create.

The tiller analyses does not include an interaction prior while the leaf analyses do.
Because of this there are two different priors set for the analyses following.
...
m3 <- foreach(1:4, .packages = "MCMCglmm") %dopar% {
  MCMCglmm(LShB_sev_AUDPS ~ WMGT * NRTE,
           random = ~ REP,
           data = AUDPS_2015,
           verbose = FALSE,
           prior = eprior,
