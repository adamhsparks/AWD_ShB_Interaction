...
# Midpoint Scale Conversion

Because the severity data were rated on an unevenly spaced ordinal scale the data are converted to the midpoint value of the percent range for disease severity index and then the AUDPC and AUDPS are calculated from those values.

```{r midpoint-conversion}
...
TShB_inc_15 <-
  ...
  summarise(PLOT_TShB_inc = mean(TShB_incidence, na.rm = TRUE),
            .groups = "keep") %>%
  arrange(PLOT)
...
TShB_inc_15_AUDPS <-
  audps(evaluation = TShB_inc_15_wide[, 2:6],
        dates = pull(TShB_inc_15[1:5, 6]))

TShB_percent_15_AUDPS <-
  audps(evaluation = TShB_perc_15_wide[, 2:6],
        dates = pull(TShB_perc_15[1:5, 6]))

LShB_percent_15_AUDPS <-
  audps(evaluation = LShB_perc_15_wide[, 2:6],
...
# Add year/rep/trt/asmt cols to AUDPS values
ShB_15 <- left_join(TShB_inc_15, AUDPS_15, by = "PLOT")
```

## 2016 Area Under Curve Calculations
...
# 2016 audpS -------------------------------------------------------------------
TShB_inc_16_AUDPS <-
  audps(evaluation = TShB_inc_16_wide[, 2:5],
        dates = pull(TShB_inc_16[1:4, 6]))

TShB_percent_16_AUDPS <-
  audps(evaluation = TShB_perc_16_wide[, 2:5],
        dates = pull(TShB_perc_16[1:4, 6]))

LShB_percent_16_AUDPS <-
  audps(evaluation = LShB_perc_16_wide[, 2:5],
