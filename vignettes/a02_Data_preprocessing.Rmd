...
Because the severity data were rated on an unevenly spaced ordinal scale the data are converted to the midpoint value of the percent range for disease severity index and then the AUDPS is calculated from those values.
...
TShB_percent_15 <-
  ...
  summarise(PLOT_TShB_percent = mean(PERCENT_TIL_ShB, na.rm = TRUE),
            .groups = "keep") %>%
  arrange(PLOT)
...
LShB_percent_15 <-
  DS2015 %>%
  group_by(YEAR, REP, TRT, PLOT, ASMT, DAI, DAYS) %>%
  summarise(PLOT_LShB_percent = mean(PERCENT_LEAF_ShB, na.rm = TRUE),
            .groups = "keep") %>%
  arrange(PLOT)
